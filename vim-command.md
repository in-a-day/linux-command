...
--- | ---
`[count]G` | 跳转到[count]行
`/pattern<CR>或?pattern<CR>` | 跳转到下一个或上一个模式出现之处
`n或N` | 跳转到下一处或上一处匹配位置
`%` | 跳转到匹配之处
`(`或`)` | 跳转到上一句或下一句开头
`{`或`}` | 跳转到上一段或下一段开头
...
### **宏**
1. `q{register}` 录制或结束录制宏到指定寄存器(即q加上寄存器名称开始录制, 再次按q结束录制)
2. `@{register}` 执行寄存器中的宏
3. `@@` 重复执行上一个宏
4. 串行执行宏时, 如果遇到错误将会中断
5. 使用`[range] normal @{register}`并行执行宏, 遇到错误将会继续执行下一行.
6. 使用`qa` 录制宏, 完成后若想要添加命令使用`qA`
7. 宏命令可以复制文本修改, 然后将文本重新保存至宏寄存器中


## **模式**
### **查找模式**
1. `:set ignorecase` 设置全局不区分大小写
2. `\c` 不区分大小写, `\C` 强制区分大小写
3. `:set smartcase` 只要输入大写字母则区分大小写, 全是小写则不区分大小写
4. `\v` 激活very magic搜索模式, 该模式下除了`_`, 大小写字母及0~9之外的字符都具有特殊含义(使vim的正则更像python)
5. `\V` 激活very nomagic原义开关, 消除`.*?`等大多数字符的特殊含义

### **匹配技巧**
1. `/\v<(\w+)_s+\1>` 匹配连续出现的单词, () 与\1匹配, \1代表重复()内容, \0默认为该行, 1即代表()的位置, 如果含有其他括号, 可使用\2, \3等等.
2. `<>` 单词定界符, 如`/\v<the>` 将仅匹配the这个单词.
3. `%()` vim将不再捕获`()`中内容

### **查找**
1. 执行完查找命令后, 可以使用`:substitute`执行替换
2. `:vimgrep //g %` 执行完查找命令后, 可以统计查找模式匹配个数和当前所在的位置, 查找模式留空为了让vimgrep使用当前的查找模式, %代表当前文件完整路径. 该命令将结果存在quickfix列表, `cnext` 与 `cpre`遍历该列表.
3. `:%s///gn` 统计匹配个数, `n`会抑制替换动作

命令 | 用途
-- | ---
`gn` | 进入面向字符的可视模式, 选中下一处匹配
`gN` | 进入面向字符的可视模式, 选中上一处匹配
`*` | 向下查找当前单词
`#` | 向上查找当前单词
...
1. `[range]` 范围, pattern模式, string替换的内容, flags标志位
2. flags: `c` 提示确认每处替换, 可以拒绝, 接受等; `g` 匹配的行全部替换; `n` 抑制正常的替换行为, 显示pattern匹配的个数; `e` 屏蔽'找不到模式'的错误; `&` 重用上次substitute的标志位;
3. {pattern}留空时默认是上一次的查找域, `<C-r>/`将查找域内容插入(因为`"/"`表示的是默认的查找寄存器)
4.  `[range]s/{pattern}/\=@{register}/[flags]` 将会使用寄存器内容替换匹配的模式, `\=`指示执行vim脚本,在vim脚本中`@{register}`会引用寄存器内容;
...
`\0` | 插入匹配模式的所有内容
`&` | 插入匹配模式的所有内容
`~` | 使用上一次调用substitute时的{string}
`\={vim script}` | 将{vim script}结果作为{string}

## **global命令**
> `[range] g[lobal][!] /{pattern}/ [cmd]`
1. 缺省状态下global命令作用范围是整个文件
2. {pattern}与查找模式相关联, 即缺省使用上次的查找模式
3. [cmd]是除global外的任何Ex命令, 缺省时使用print命令
4. `:g[lobal]!` 和 `:v[global]`反转 global命令
5. `:global`命令在指定[range]上执行两轮, 首先在匹配的[pattern]行上做标记, 然后在所有标记的匹配上执行[cmd]命令. [cmd]命令的范围可以单独指定.
- 技巧:
    - `[range] g /{pattern}/ delete` 删除指定[range]中匹配的全部行
    - `[range] g! /{pattern}/ delete` 删除指定[range]中不匹配的全部行
